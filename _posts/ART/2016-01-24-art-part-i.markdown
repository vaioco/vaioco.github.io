...
OAT file format is developed by Google, it is embedded into an ELF. An OAT file contains DEX code, compiled native code and metadata. OAT files are loadedin application virtual memory like a shared object.

Android framework JARs are compiled at the first device boot (maybe OEM devices can act differently), the resulting files are _boot.art_ (the **image file**) and _boot.oat_.
...
_ArtMethod_ is the C++ class which define the Java methods memory-layout, its interesting elements are: 
...
I'm going to use GDB as debugger, radare is a very good alternative, with AOSP symbols. Create a directory with links to AOSP symbols, like the following:
