...
Android framework JARs are compiled at the first device boot (OEM devices can act different, boot image is precompiled inside the firmware) , the resulting files are _boot.art_, the **image file**, and _boot.oat_.
